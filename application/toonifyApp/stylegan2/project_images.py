...
def main():
    #src_dir = Path(__file__).resolve().parent.parent/"stylegan2/aligned"
    src_dir = Path(__file__).resolve().parent.parent/"stylegan2/aligned"
    dst_dir = Path(__file__).resolve().parent.parent/"stylegan2/generated"
    tmp_dir = Path(__file__).resolve().parent.parent/"stylegan2/stylegan2-tmp"
    ...
    #parser.add_argument('--src_dir', default='aligned',help='Directory with aligned images for projection')
    #parser.add_argument('--dst_dir', default='generated',help='Output directory')
   ...
   # print('Loading networks from "%s"...' % args.network_pkl)
    blended_url = Path(__file__).resolve().parent.parent/"../static/libs/stylegan2-ffhq-config-f.pkl"
    _G, _D, Gs = pretrained_networks.load_networks(blended_url)
  #  _G, _D, Gs = pretrained_networks.load_networks(args.network_pkl)
    ...
    for src_file in src_files:
        project_image(proj, src_file, dst_dir, tmp_dir, video=_video)
        #if video:
        #    render_video(
